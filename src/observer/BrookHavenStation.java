...
/**
 * Class containing information pertaining to BrookHavenStation
 * 
 * @author devang
 *
 */
public class BrookHavenStation implements StationObserver {

	//private Stations st;
	private int relativex;
	private int relativey;
	
    public BrookHavenStation(Stations st) {      
          st.addObserver( this );
          relativex = 0;
          relativey = 0;
    }
	...
	public boolean update(StationObserver ar, int x, int y) {
		...
		if(Math.abs(StationUtils.relativeX(x)) <=2) {
			...
			if(ar instanceof BrookHavenStation) {
				System.out.println("Current Location -> Jackson");
				System.out.println("BrookHaven Station reports " + AircraftObservable.aircraftID + 
						" at " + stationLocation() + " (" + StationUtils.relativeX(x) +
