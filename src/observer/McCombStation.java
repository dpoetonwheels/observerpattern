...
/**
 * Class containing information pertaining to McCombStation
 * 
 * @author devang
 *
 */
public class McCombStation implements StationObserver {

	private int relativex;
	private int relativey;
	public McCombStation(Stations st) {
	...
	public boolean update(StationObserver ar, int x, int y) {
		...
		if(Math.abs(StationUtils.relativeX(x)) <=2) {
			...
			if(ar instanceof McCombStation) {
				System.out.println("Current Location -> MS");
				System.out.println("McComb Station reports " + AircraftObservable.aircraftID + " at " +
